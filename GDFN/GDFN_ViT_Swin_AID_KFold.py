...
Usage  : python GDFN_ViT_Swin_AID_KFold.py
"""
...
x_train_d=extract_distances(x_train); x_valid_d=extract_distances(x_valid); x_test_d=extract_distances(x_test)

x_train/=255.0; x_valid/=255.0; x_test/=255.0
x_train_d/=255.0; x_valid_d/=255.0; x_test_d/=255.0
y_train=np.array(y_train); y_valid=np.array(y_valid); y_test=np.array(y_test)

x_all=np.concatenate((x_train,x_valid)); x_all_d=np.concatenate((x_train_d,x_valid_d)); y_all=np.concatenate((y_train,y_valid))
...
for name,timm_name in MODELS.items():
    ...
    for tr_idx,val_idx in kf.split(x_all):
        ...
        for epoch in range(70):
            ...
            print(f"    Ep {epoch+1:2d}/70  loss={tl:.4f}  train={ta:.4f}  val={accuracy_score(vt,vp):.4f}")
        torch.save(model.state_dict(),os.path.join(MODEL_SAVE_DIR,f'GDFN_{name}_fold{fold_no}.pt'))
        y_pred,y_true=evaluate(model,test_loader)
        print(f"\n  Results — GDFN {name} Fold {fold_no}:")
